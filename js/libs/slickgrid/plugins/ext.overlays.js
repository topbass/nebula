(function ($) {
    ...
    function Overlays(options) {
        ...
        function gridScrolled(e, args) {
            // June 13, 2014 by Rollie
            // added setTimeout() to delay overlay move functions in the event loop,
            // in order to fix resizeCanvas() and transitionEnd event triggering issue
            setTimeout(function() {
                moveHeaderRowOverlays(e, args);
                moveSelectionOverlay(e, args);
            }, 0);
        ...
        function moveSelectionOverlay(e, args) {
            ...
            // ----- Patch added by Rollie Ma on Jul 29, 2014 -----
            if (position === null) {
                var rPos = $(grid.getActiveCellNode()).parent().position(),
                    cPos = $(grid.getActiveCellNode()).position();
                position = {
                    top: rPos.top,
                    left: cPos.left,
                    bottom: rPos.top + grid.getOptions().rowHeight - 1,
                    right: cPos.left + column.width
                };
            }
            // ----- Patch added by Rollie Ma on Jul 29, 2014 -----
